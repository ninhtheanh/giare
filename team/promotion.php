...
$mark = $now - ($now % 60);
//DB::$mDebug=true;
$where = "WHERE `team_type` = 'normal' AND city_id IN(556,0,".abs(intval($city['id'])).") AND begin_time <= $mark AND end_time > $now 
...
if (!option_yes('displayfailure')) {
    ...
    $where .= " AND (now_number >= 100 OR end_time > $mark)";
}
$sqlCount = "SELECT count(1) as count_item FROM `team`, promotion_product $where";
$count_item = DB::GetQueryResult($sqlCount); $count = $count_item["count_item"];
list($pagesize, $offset, $pagestring) = pagestring($count, 1000);
$sql = "SELECT team.* FROM `team`, promotion_product $where";
