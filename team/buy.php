	...
	if ($flag = $table->update($insert)) {
		...
		if($id==$id_promotion){
			...
			if(empty($check['verify_code'])){
				...
				$subject = "{$INI['system']['abbreviation']} - Xác nhận tham gia chương trình \"Miễn phí 01 năm mua sắm trên web: www.cheapdeal.vn\"";
...
//var_dump(CheckCityDeal($team['city_id']));
