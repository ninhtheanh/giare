...
if ( !$team || $team['begin_time']>time() ) {
	Session::Set('error', 'deal doesnt exist');
	redirect( 'index.php' );
}
team_state($team);
if ($team['close_time']) {
	Session::Set('error', 'deal closed，you cant buy it now');
	redirect( 'index.php' );
}
...
if (strtoupper($team['buyonce'])=='Y') {
	...
	if ( Table::Count('order', $ex_con) ) {
		Session::Set('error', 'your have bought this product, please have a look at other deals！');
		redirect( "now.php"); 
	}
}
...
if ($team['per_number']>0) {
	...
	if ($team['per_number']<=0) {
		Session::Set('error', 'your purchase of this deal has met the upper limit, please have a look at other deals！');
		redirect( 'now.php' ); 
	}
}
...
if ( $_POST ) {
	...
	elseif ( $team['per_number']>0 && $table->quantity > $team['per_number'] ) {
		Session::Set('error', 'your purchase of this ideal has met the upper limit！');
	...
	if ($team['delivery']=='express') {
		if (!$table->address 
			...
			Session::Set('error', 'your info of order is not complete');
