...
//payed order
if ( $order['state'] == 'pay' ) {  
	Session::Set('notice', 'You have paid this deal');
...
if ($login_user['money'] >= $order['origin']) { 
	...
	Session::Set('error', 'account balance isnt enough, please recharge your account online first！');
	redirect('index.php');
}
...
if ( $_POST['service'] == 'credit' ) {
	...
	ZTeam::BuyOne($order);
	Session::Set('notice', 'purchase succeeded');
