...
try {
    ...
    if(!$openid->mode) {
		...
		if($uu){
			$login_user = ZUser::GetUserByYahoo_IdMail('yes',$uu['contact/email']);
			if($login_user) {
				if($login_user['is_yahoo']=='no') {
					//update
					$sql = "UPDATE `user` SET is_yahoo = 'yes' WHERE id ='".$login_user['id']."'";
					mysql_query($sql);
					//login
				...
				$_SESSION['YAHOO_USER_LOGIN'] = $u;
				$openid->returnUrl = WEB_ROOT .'/account/signupyahoo.php';
