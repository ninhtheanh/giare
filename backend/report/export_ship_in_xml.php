...
$id = abs(intval($_GET['in_id']));
$q = DB::GetQueryResult("SELECT
ship_in.in_id,
ship_in.out_id AS Ma_BG,
ship_in.created AS Ngay_BG,
ship_in.approved AS Ngay_NT,
ship_in.shipper AS Ma_NV_GH,
ship_in.creator AS Ma_NV_BG,
ship_in_data.deal_id AS Ma_Voucher,
team.short_title AS Ten_Voucher,
ship_in_data.price AS Don_Gia,
ship_in_data.pay AS SL_Giao,
ship_in_data.pay AS SL_Ban,
(ship_in_data.price*ship_in_data.quantity) AS Thanh_Tien,
(ship_in_data.price*ship_in_data.pay) AS Thuc_Thu
FROM ship_in
INNER JOIN ship_in_data ON ship_in_data.in_id = ship_in.in_id
INNER JOIN team ON ship_in_data.deal_id = team.id", false);
...
$aa = DB::GetQueryResult("UPDATE `ship_in` SET exported='Y' WHERE in_id=".$q[i]['in_id']);
    ...
    <Cell ss:StyleID="s73"><Data ss:Type="Number">'.$qq['tien_km'].'</Data></Cell>
    <Cell ss:StyleID="s73"><Data ss:Type="Number">'.$qq['tien_tra_truoc'].'</Data></Cell>
