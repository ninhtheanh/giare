...
$condition = array(
	/*'team_id' => $t_id,*/
	'team_id > 0',
	"state IN ('canceled', 'dealsoc_cancel')",
);
/* filter */
...
$demail = strval($_GET['duser']);
if($demail){
	$q = DB::GetQueryResult("SELECT order_id FROM order_history WHERE `user`='".$demail."' AND `status_code` = 'canceled'", false);

	foreach($q as $i => $j){
		$list_oid .= $j['order_id'].",";	
	}
	$condition[] = "id IN (".rtrim($list_oid,",").")";
}else $demail = null;
...
include template('manage_order_canceled');
