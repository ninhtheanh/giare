...
if ($sortby) { 
	...
	//$order_by = "ORDER BY stt_bbbg ASC, id DESC";
	$order_by = "ORDER BY bstreet_name, bdist_id, bward_id ASC, id DESC";
}
...
if($_GET['delivery_code']>0){
	...
	if((time()<$accept_ship_in_time) && (!in_array($login_user['id'],$accept_id_pay))){
		...
		list($pagesize, $offset, $pagestring) = pagestring($count, 400);
...
{		
	...
	for($i=0;$i<count($id);$i++){
		...
		$order_note = $_POST["order_notes{$id[$i]}"];
		...
		if($order_note>0){
			$mm = DB::GetQueryResult("SELECT title FROM notes WHERE id=".$order_note);
		}else{
			$mm = "";
		}
		if($order_note==5){
		...
		if($row!=''){
			...
			save_order_history($id[$i],$row,'',rtrim($note.", ".$mm['title'],", "));
			//update ship_out.notes
			//echo "UPDATE ship_out_data SET state='".$row."', service='".$service."', notes='".trim($note)."', qty_successful='".$qty_success."', date_received='".$date_received."' WHERE order_id='".$id[$i]."' AND out_id='".$delivery_code."';<br>";
			DB::Query("UPDATE ship_out_data SET state='".$row."', service='".$service."', notes='".add_input(trim($note))."', qty_successful='".$qty_success."', date_received='".$date_received."' WHERE order_id='".$id[$i]."' AND out_id='".$delivery_code."'");
			$vv = DB::GetQueryResult("SELECT id FROM order_notes WHERE order_id=".$id[$i]." AND out_id=".$delivery_code);

			if($order_note>0 && (int)$vv['id']==0){
				DB::Query("INSERT INTO order_notes(order_id, notes_id, user_id, created, out_id ) VALUES('".$id[$i]."', '".add_input($order_note)."', '".$login_user_id."', '".date('Y-m-d H:i:s',time())."', '".$delivery_code."')");
			}else{
				DB::Query("UPDATE order_notes SET order_id='".$id[$i]."', notes_id='".add_input($order_note)."', user_id='".$login_user_id."', created='".date('Y-m-d H:i:s',time())."', out_id='".$delivery_code."' WHERE id=".$vv['id']);
			}
