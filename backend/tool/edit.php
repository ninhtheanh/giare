...
if ( $_POST && $id==$_POST['id'] && $shipper) {
	...
	$up_array = array(
			'shipper_id', 'shipper_name', 'shipper_address', 'shipper_status', 'user_id','shipper_tel'
			);

	$flag = $table->update( $up_array );
	if ( $flag ) {
		Session::Set('notice', 'Modify Shipper Information Success');
		redirect( WEB_ROOT . "/backend/shipper/edit.php?id={$id}");
	}
	Session::Set('error', 'Shipper information failed to modify');
	$partner = $_POST;
}

include template('manage_shipper_edit');
