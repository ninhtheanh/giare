...
<head>
    ...
    <title> Deal Soc | Deals |deal khuyến mãi deal gía rẻ, mua hàng theo nhóm dealsoc trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?> - <?php echo $city['name']; ?></title>
    <?php } else if($team) { ?>
    <title><?php echo $team['short_title']; ?> <?php echo ($seo_title) ? ' '.$seo_title : '';; ?> | <?php echo ($_GET['comment']) ? 'thảo luận deal - ' : '';; ?><?php echo $INI['system']['sitename']; ?> | phiếu khuyến mãi giảm giá cực rẻ</title>    
     <?php } else if(strpos($_SERVER['REQUEST_URI'],'/team/')!==false) { ?>
  	...
  	<?php }?>
    <?php if(!$pagetitle||$request_uri=='index'){?>
    	 <meta name="description" content="nhóm Mua hàng theo nhóm deals sản phẩm, deal dịch vụ giảm giá, deals mua sắm, deal du lịch, deals làm đẹp, deal ăn uống, deals nhà hàng, deals thời trang, deal spa và các deals sản phẩm khuyến mãi độc đáo tại Dealsoc - trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?> - <?php echo $city['name']; ?>" />
