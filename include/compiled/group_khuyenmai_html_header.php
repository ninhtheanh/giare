...
<head>
    ...
    <title> Deal Soc | Khuyến mãi | Nhóm mua chung hotdeal, cùng mua deal khuyến mãi deal gía rẻ, mua hàng theo nhóm dealsoc</title>
    <?php } else if($team) { ?>
    <title><?php echo $team['short_title']; ?> <?php echo ($seo_title) ? ' '.$seo_title : '';; ?> | <?php echo ($_GET['comment']) ? 'thảo luận deal - ' : '';; ?><?php echo $INI['system']['sitename']; ?> | phiếu khuyến mãi giảm giá cực rẻ</title>    
     <?php } else if(strpos($_SERVER['REQUEST_URI'],'/team/')!==false) { ?>
  	...
  	<?php }?>
    <?php if(!$pagetitle||$request_uri=='index'){?>
    	 <meta name="description" content="Dealsoc khuyến mãi mua hàng theo nhóm hot deals Cùng nhóm mua chung deal khuyến mãi sản phẩm, khuyến mãi dịch vụ giảm giá, khuyến mãi mua sắm, khuyến mãi du lịch, khuyến mãi làm đẹp, khuyến mãi ăn uống, khuyến mãi nhà hàng, khuyến mãi thời trang, khuyến mãi spa và các khuyến mãi sản phẩm khuyến mãi độc đáo tại Dealsoc.vn - trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?>" />
    ...
    <meta http-equiv="X-UA-Compatible" content="IE=9" />
      <?php if(!$pagetitle||$request_uri=='index'){?>
     	<meta name="keywords" content="khuyenmai, khuyen mai, dealsoc, deal soc, deal, deals, nhommua, hotdeal, hotdeals, cungmua, muachung, nhom mua, cung mua,mua chung,mua theo nhom, mua hang theo nhom, giam gia, uu dai, coupon, gia re, deal hôm nay, <?php echo $city['name']; ?>" />
