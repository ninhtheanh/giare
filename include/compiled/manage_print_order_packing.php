  ...
  <div id="bd" class="cf">
    <div id="coupons">
      <div class="subdashboard" id="dashboard">
        <ul>
          <?php echo mcurrent_order('printorderpacking'); ?>
        </ul>
      </div>
        ...
        <div class="box clear">
          ...
          <div class="box-content"> 
                ...
                <form name="frmsearch" method="POST">
                  <tr>
                    ...
                    <td align="left"><select name="block" id="block" onChange="document.frmsearch.submit()">
                        ...
                        <?php if(is_array($ds)){foreach($ds AS $index=>$val) { ?>
                        <option value="<?php echo $val['time_process']; ?>"><?php echo date('d-m-Y H:i:s',strtotime($val['time_process'])); ?></option><?php }}?>
                      </select>
                      <script type="text/javascript">$("#block").val("<?php echo $time_processing; ?>");</script></td>
                  </tr>
                </form>
              </table>
            </div>
            <form name="frmprintorderpacking" id="frmprintorderpacking" method="POST" action="/backend/print_order_packing.php" target="_blank" onsubmit="return CheckSubmit()">
              ...
              <input type="hidden" name="timeprocessing" value="<?php echo $time_processing; ?>">
      ...
      ; ?>      
        ...
        Địa chỉ: <?php echo $one['billing_address']; ?><br />ĐT: <?php echo $one['mobile']; ?><br /><span style="color:#c40000"><strong><?php echo GetPaymentName($one['payment_id']); ?></strong></span></td>
                    <td>Họ tên: <a href="/ajax/manage.php?action=userview&id=<?php echo $one['user_id']; ?>" class="ajaxlink"><strong><?php if($one['brealname']!=''){?><?php echo $one['brealname']; ?><?php } else { ?><?php echo $one['realname']; ?><?php }?></strong></a><br />
