...
<head>
    ...
    <title> Deal Soc | nhóm mua deal khuyến mãi deal gía rẻ - <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?> - <?php echo $city['name']; ?></title>
    <?php } else if($team) { ?>
    <title><?php echo $team['short_title']; ?> <?php echo ($seo_title) ? ' '.$seo_title : '';; ?> | <?php echo ($_GET['comment']) ? 'thảo luận deal - ' : '';; ?><?php echo $INI['system']['sitename']; ?> | phiếu khuyến mãi giảm giá cực rẻ</title>    
     <?php } else if(strpos($_SERVER['REQUEST_URI'],'/team/')!==false) { ?>
  	...
  	<?php }?>
    <?php if(!$pagetitle||$request_uri=='index'){?>
    	 <meta name="description" content="nhóm Mua hàng theo nhóm cùng nhóm mua chung sản phẩm, nhom mua dịch vụ giảm giá, nhom mua sắm, nhom mua du lịch, nhom mua làm đẹp, nhom mua ăn uống, nhom mua nhà hàng, nhom mua thời trang, nhom mua spa và các nhom mua sản phẩm khuyến mãi độc đáo tại Dealsoc - trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?> - <?php echo $city['name']; ?>" />
    ...
    <meta http-equiv="X-UA-Compatible" content="IE=9" />
      <?php if(!$pagetitle||$request_uri=='index'){?>
     	<meta name="keywords" content="deal, deals, nhom mua, nhommua, dealsoc , hotdeal, hotdeals, cungmua, muachung, cung mua,mua chung,mua theo nhom, mua hang theo nhom, giam gia, uu dai, coupon, khuyen mai, gia re, deal hôm nay, trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?> ,<?php echo $city['name']; ?>"
