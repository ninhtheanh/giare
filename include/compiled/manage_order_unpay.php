    ...
    <div id="content" class="coupons-box clear mainwide">
		<div class="box clear">
            <div class="subbox-top"><div class="subhead">
					...
					<ul class="filter"><?php echo current_manageorder('', 0); ?></ul>
				</div></div>
					...
					<form method="get">
                    	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="filter-table">
                          <tr>
                            ...
                            <td width="4%" align="right"><strong>Deal No.</strong></td>
                          ...
                          <tr>
                            ...
                            <td align="right" nowrap="nowrap" style="padding-left:3px;"><strong>Pay time:</strong></td><td><input type="text" class="h-input" onFocus="WdatePicker({isShowClear:false,readOnly:true})" name="pbday" value="<?php echo $pbday; ?>" /></td><td align="center">-</td><td><input type="text" class="h-input" onFocus="WdatePicker({isShowClear:false,readOnly:true})" name="peday" value="<?php echo $peday; ?>" /></td><td style="padding-left:10px;"><p style="margin:5px 0;"><input type="submit" value="Lọc dữ liệu" class="formbutton"  style="padding:3px 6px;"/></p></td>
                          </tr>
                        </table>
					<form>
                <div class="sect">
					<table id="orders-list" cellspacing="0" cellpadding="0" border="0" class="coupons-table">
					<tr><th width="20">ID</th><th width="400">deal</th><th width="300">user</th><th width="20" nowrap>q.ty</th><th width="60" nowrap>total (<span class="money"><?php echo $currency; ?></span>)</th><!--<th width="60" nowrap>Unpaid (<span class="money"><?php echo $currency; ?></span>)</th><th width="60" nowrap>due (<span class="money"><?php echo $currency; ?></span>)</th><th width="120" nowrap>express</th>--><th width="110" nowrap>operation</th></tr>
					...
					<tr <?php echo $index%2?'':'class="alt"'; ?> id="order-list-id-<?php echo $one['id']; ?>">
                        ...
                        <br /><span style="color:#c40000"><strong><?php echo GetPaymentName($one['payment_id']); ?></strong></span><!--<br /><img src="/backend/barcode/?barcode=<?php echo $one['id']; ?>" border="0" alt="" />--></td>
						...
						<td><?php echo print_price(moneyit($one['origin']-$one['credit'])); ?></td>-->
						<!--<td><?php echo $option_delivery[$teams[$one['team_id']]['delivery']]; ?><?php echo $one['express_id']?'Y':''; ?><?php echo $service[$one['service']]; ?></td>-->
						<td align="center"><a href="/ajax/manage.php?action=orderedit&id=<?php echo $one['id']; ?>" class="ajaxlink">edit</a> | <a href="/ajax/manage.php?action=orderview&id=<?php echo $one['id']; ?>" class="ajaxlink">detail</a> | <a href="/ajax/manage.php?action=orderconfirm&id=<?php echo $one['id']; ?>" class="ajaxlink">confirm</a> | <a href="/ajax/manage.php?action=ordercancel&id=<?php echo $one['id']; ?>" class="ajaxlink" ask="sure to cancel it?">cancel</a> | <a href="/ajax/manage.php?action=ordertransfer&id=<?php echo $one['id']; ?>" class="ajaxlink">transfer</a> | <a href="/ajax/manage.php?action=orderoffice&id=<?php echo $one['id']; ?>" class="ajaxlink">Office | <a href="/ajax/manage.php?action=orderhanging&id=<?php echo $one['id']; ?>" class="ajaxlink">Tạm hoãn</a><!-- | <a href="/ajax/manage.php?action=orderremove&id=<?php echo $one['id']; ?>" class="ajaxlink" ask="sure to delete this order?">delete</a>--></td>
					</tr>
					<?php }}?>
