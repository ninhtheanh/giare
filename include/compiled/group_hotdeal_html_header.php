...
<head>
    ...
    <title> Deal Soc | hot deal nhóm mua chung deal khuyến mãi deal giá rẻ trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?> - <?php echo $city['name']; ?> </title>
    <?php } else if($team) { ?>
    <title><?php echo $team['short_title']; ?> <?php echo ($seo_title) ? ' '.$seo_title : '';; ?> | <?php echo ($_GET['comment']) ? 'thảo luận deal - ' : '';; ?><?php echo $INI['system']['sitename']; ?> | phiếu khuyến mãi giảm giá cực rẻ</title>    
     <?php } else if(strpos($_SERVER['REQUEST_URI'],'/team/')!==false) { ?>
  	...
  	<?php }?>
    <?php if(!$pagetitle||$request_uri=='index'){?>
    	 <meta name="description" content="Mua hàng theo nhóm hot deal Cùng mua hot deal sản phẩm, hot deals dịch vụ giảm giá, hot deal mua sắm, hot deal du lịch, hotdeal làm đẹp, hot deals ăn uống, hotdeals nhà hàng, hot deal thời trang, hotdeal spa và các hot deals sản phẩm khuyến mãi độc đáo tại Dealsoc - trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?> - <?php echo $city['name']; ?>" />
    ...
    <meta http-equiv="X-UA-Compatible" content="IE=9" />
      <?php if(!$pagetitle||$request_uri=='index'){?>
     	<meta name="keywords" content="deal, deals, hotdeal, hotdeals, hot deal, hot deals, nhommua, cungmua, muachung, nhom mua, cung mua,mua chung,mua theo nhom, mua hang theo nhom, giam gia, uu dai, coupon, khuyen mai, gia re, deal hôm nay,dealsoc, trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?>,<?php echo $city['name']; ?>" />
