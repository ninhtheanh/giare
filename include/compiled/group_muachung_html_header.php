...
<head>
    ...
    <title> Deal Soc | cùng mua chung deal khuyến mãi giá rẻ mỗi ngày tại dealsoc.vn | Dealsoc hôm nay trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?> - <?php echo $city['name']; ?> </title>
    <?php } else if($team) { ?>
    <title><?php echo $team['short_title']; ?> <?php echo ($seo_title) ? ' '.$seo_title : '';; ?> | <?php echo ($_GET['comment']) ? 'thảo luận deal - ' : '';; ?><?php echo $INI['system']['sitename']; ?> | phiếu khuyến mãi giảm giá cực rẻ</title>    
     <?php } else if(strpos($_SERVER['REQUEST_URI'],'/team/')!==false) { ?>
  	...
  	<?php }?>
    <?php if(!$pagetitle||$request_uri=='index'){?>
    	 <meta name="description" content="Mua chung theo nhóm, Cùng nhóm mua chung sản phẩm, dịch vụ, du lịch, làm đẹp, ăn uống, mua chung nhà hàng, mua chung thời trang, mua chung spa và các mua chung sản phẩm khuyến mãi độc đáo tại Dealsoc - trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?> - <?php echo $city['name']; ?>" />
    ...
    <meta http-equiv="X-UA-Compatible" content="IE=9" />
      <?php if(!$pagetitle||$request_uri=='index'){?>
     	<meta name="keywords" content="mua chung, muachung, dealsoc, deal soc, deals,nhommua, hotdeal, hotdeals, cungmua, muachung, nhom mua, cung mua, mua theo nhom, mua hang theo nhom, giam gia, uu dai, coupon, khuyen mai, gia re, deal hôm nay, trang <?php echo ($_GET['page']) ? $_GET['page'] : 1;; ?>, <?php echo $city['name']; ?>" />
