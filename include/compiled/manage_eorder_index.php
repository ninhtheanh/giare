...
<div class="sect">
  <table id="orders-list" cellspacing="0" cellpadding="0" border="0" class="coupons-table">
    <tr>
      <th width="20">Ma GH</th>
      <th width="100">ID</th>
      <th width="350">Thông tin thanh toán</th>
      <th width="300">Thông tin vận chuyển</th>
      <th width="55" nowrap>Thành tiền</th>
      <th width="95" nowrap>&nbsp;</th>
    </tr>
    ...
    <tr  style="background-color:#<?php echo getStatusColor($one['state']); ?>" id="order-list-id-<?php echo $one['id']; ?>">
      <td style="color:#fff;background:<?php echo toColor($one['create_time']+$one['user_id']); ?>"><strong><strong><?php echo  $one['order_group']; ?></strong></strong><br />
						<?php echo $one['id']; ?></td>
      <td>Deal: <strong><?php echo $one['team_id']; ?>&nbsp;(<a class="deal-title" href="/<?php echo $city['slug']; ?>/<?php echo seo_url($teams[$one['team_id']]['short_title'],$one['team_id'],$url_suffix); ?>" target="_blank"><?php echo $teams[$one['team_id']]['short_title']; ?>
      <?php echo showColorSize($one); ?>
      )
      </strong><br />Số đ.hàng: <a href="/backend/order/orderedit.php?id=<?php echo $one['id']; ?>&redirect=/backend/order/index.php?city_id=556" title="Sửa thông tin đơn hàng"><strong><?php echo $one['id']; ?></strong></a><br />
        <span style="white-space:nowrap">Ngày đặt: <?php echo date('Y-m-d H:i:s',$one['create_time']);; ?></span><br /><span style="white-space:nowrap">Trạng thái: <strong style="color:#0000FF;"><?php echo getStatusName($one['state']); ?></strong></span></td>
      <td>Họ tên: <a href="/ajax/manage.php?action=userview&id=<?php echo $one['user_id']; ?>" class="ajaxlink"><strong><?php echo $one['realname']; ?></strong></a><br />
		...
		<?php 
            $billing_address = ""; $note_address = "";
            if($one['note_address']!=''){
                $note_address = htmlspecialchars($one['note_address']).", ";
            }
            $billing_address .= $note_address.htmlspecialchars($one['address']);
            if(ward_name($one['dist_id'],$one['ward_id'])){
                $wardname = ward_name($one['dist_id'],$one['ward_id']);
                $billing_address .=", ".strip_input($wardname['ward_name']);
            }
            if(ename_dist($one['dist_id'])){
                $dists = ename_dist($one['dist_id']);
                $billing_address .=", ".strip_input($dists['dist_name']);		
            }
            if(id_city($one['city_id'])){
                $citys = id_city($one['city_id']);
                $billing_address .=", ".strip_input($citys['name']);		
            }
            
            $shipping_address = "";$note_baddress="";
            if($one['bnote_address']!=''){
                $note_baddress = htmlspecialchars($one['bnote_address']).", ";
            }
            $shipping_address .= $note_baddress.htmlspecialchars($one['baddress']);
            if(ward_name($one['bdist_id'],$one['bward_id'])){
                $bwardname = ward_name($one['bdist_id'],$one['bward_id']);
                $shipping_address .=", ".strip_input($bwardname['ward_name']);
            }
            if(ename_dist($one['bdist_id'])){
                $bdists = ename_dist($one['bdist_id']);
                $shipping_address .=", ".strip_input($bdists['dist_name']);		
            }
            if(id_city($one['bcity_id'])){
                $bcitys = id_city($one['bcity_id']);
                $shipping_address .=", ".strip_input($bcitys['name']);		
            }
        
      ; ?>      
      
        Địa chỉ: <?php echo $billing_address; ?><br />ĐT: <?php echo $one['mobile']; ?><br /><span style="color:#c40000"><strong><?php echo GetPaymentName($one['payment_id']); ?></strong></span></td>
      <td>Họ tên: <a href="/ajax/manage.php?action=userview&id=<?php echo $one['user_id']; ?>" class="ajaxlink"><strong><?php echo $one['brealname']; ?></strong></a><br />
        Địa chỉ: <?php echo $shipping_address; ?><br />Điện thoại: <?php echo $one['bmobile']; ?><br /><span style="color:#c40000"><strong><?php echo GetShippingName($one['shipping_id']); ?></strong></span></td>
      <td align="right" style="color:#FF0000"><strong><?php echo print_price(moneyit(TotalOrder($one['id']))); ?></strong></td>
      <?php /* <td align="center"><?php if(total_order_user($users[$one['user_id']]['id'])>0){?><b style="color:#FA6D18"><?php echo $old_cus='KH cũ'; ?></b><?php } else { ?><b style="color:#C40000"><?php echo $old_cus='KH mới'; ?></b><?php }?><br /><a href="/ajax/manage.php?action=orderview&id=<?php echo $one['id']; ?>" class="ajaxlink"><strong>Detail</strong></a><br /><a href="/ajax/manage.php?action=orderconfirm&id=<?php echo $one['id']; ?>" class="ajaxlink"><strong>Confirm</strong></a><br /><a href="/ajax/manage.php?action=ordercancel&id=<?php echo $one['id']; ?>" class="ajaxlink" ask="sure to cancel it?"><strong>Cancel</strong></a></td> */ ?>
