...
class DB
{
	...
	function __construct()
	{
		...
		self::$mConnection = mysql_connect( $host, $user, $pass ) or die ('connect db failed.');
	...
	static public function Query( $sql )
	{
		...
		if ( $result )
		{
			//self::Query("INSERT INTO sqlstr (`str`,`ref`) VALUES ('$sql','".$_SERVER['REQUEST_URI']."') ON DUPLICATE KEY UPDATE cnt=cnt+1");
			return $result;
		}
		else
		{
	...
	{
		global $m;		
		$tbcache = array('`category`','`district`','`ward`','`team`','`page`','`topic`','`topic_reply`','`friend_link`','`partner`','`shipper`','`order_status`','`address_list`','`maillist`');
		/*
		if(strpos($sql,'begin_time <')!==false)
		{
		...
		*/
		...
		if(CACHER)
		{
			switch(CACHER)
			{
				case 'X' :
					$ret = xcache_get($key);					
					if(sizeof($ret)>0 && $ret!='NULL') return $ret;							
				break;
		...
		if (CACHER && sizeof($ret)>0 && $ret!='NULL')
		{	
			foreach($tbcache as $v)
			{
				if(strpos($sql,$v)!==false) $becache = true;	
			}
			if(strpos($sql,'>')!==false) $becache = false;	
			if(strpos($sql,'<')!==false) $becache = false;
	...
	static public function GetQueryResult( $sql, $one=true, $cache=0 )
	{
		...
		//echo $sql.'<br>';		
		//echo $becache.'<br>';
		//if(strpos($_SERVER['REQUEST_URI'],'manage')!==false) $becache = false;	
		//if cached - return from cache
		if(CACHER)
		{
			switch(CACHER)
