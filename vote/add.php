...
if ($vote_today) {
	Session::Set('notice', 'You have participated in todays survey!');
...
if (!count($vote_list)) {
	Session::Set('error', 'Content must not be empty。');
	redirect( WEB_ROOT . '/vote/index.php');	
}
...
if (!$feedback_id) {
	Session::Set('error', 'adding failed，database operation failed。');
	redirect( WEB_ROOT . '/vote/index.php');	
}
...
if (is_post()) {
	Session::Set('notice', 'data submission succeeded, thank you。');
}
redirect( WEB_ROOT . '/vote/index.php');	
