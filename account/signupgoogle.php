...
if (isset($_POST['registergoogle'])) {
	$user_details = array();
	$user_details = $_SESSION['GOOGLE_USER_LOGIN'];
	unset($_SESSION['GOOGLE_USER_LOGIN']);
	...
	if(!Utility::ValidEmail($_POST['email'], true)){
		...
		Utility::Redirect( WEB_ROOT . '/account/signupgoogle.php');
	}
	...
	if($user_details) {
		$user_details['email'] = $enc->encrypt(ZUser::SECRET_KEY, $_POST['email']);
		$user_details['username'] = $enc->encrypt(ZUser::SECRET_KEY, $_POST['email']);
		...
		$user_details['is_google'] = 'yes';
		$user_id = DB::Insert('user', $user_details);
		//if($user_id = ZUser::Create($user_details)) {
		if($user_id =DB::Insert('user', $user_details)) {
