...
if (!$total_money && $action != 'redirect') {
	Session::Set('error', 'value recharged is less than 1');
...
elseif ( $order_service == 'bill' ) {		
	...
	$merchant_param = "";		///Business私有参数

	$pemail=$login_user['email'];		///传递email到快钱网关页面
	$pid="";		///代理/合作伙伴Business编号
	...
	///快钱通过服务器连接的方式将交易结果发送到[bgUrl]对应的页面地址，在Business处理完成后输出的<result>如果为1，页面会转向到<redirecturl>对应的地址。
	...
	//同一订单禁止重复提交标志
	///固定选择值： 1、0
	///1代表同一订单号只允许提交1次；0表示同一订单号在没有支付成功的前提下可重复提交多次。默认为0建议实物购物车结算类Business采用0；虚拟产品类Business采用1
	$redoFlag="0";
