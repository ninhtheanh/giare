...
// ----- Defines common functions used
	...
	function SendResponse($status, $data)
	{
		...
		if ($status) {
			...
			echo "<errormessage>" . htmlspecialchars($data, ENT_QUOTES, SENDSTUDIO_CHARSET) . "</errormessage>\n";
		}
...
switch ($handlerMethod) {
	...
	default:
		...
		if (is_object($handlerObject)) {
			if (!is_callable(array($handlerObject, $handlerMethod))) {
				SendResponse(false, 'Invalid request type');
			}

			$response = call_user_func_array(array($handlerObject, $handlerMethod), $function_params);
		} else {
			if (!is_callable("{$handlerObject}::{$handlerMethod}")) {
				SendResponse(false, 'Invalid request type');
			}

			$response = call_user_func_array("{$handlerObject}::{$handlerMethod}", $function_params);
		}
