			...
			<td>
				<table border="0" cellspacing="0" cellpadding="2" width="100%" class="Panel">
					...
					<tr class="hideOnSystemAdminSelect">
						<td class="FieldLabel">
							{template="Required"}
							{$lang.UsersGroups_Field_Access_MailingLists}:
						</td>
						<td>
							<select name="record[listadmin]" class="Field450 groupRecord_Access_Resource_Selector" {if is_array($record.permissions.system) && (in_array('system', $record.permissions.system) || in_array('list', $record.permissions.system))}disabled="disabled"{/if}>
								<option value="1" {if $record.listadmin}selected="selected"{/if}>{$lang.UsersGroups_access_mailinglists_all}</option>
								<option value="0" {if !$record.listadmin}selected="selected"{/if}>{$lang.UsersGroups_access_mailinglists_custom}</option>
							</select>
							{$lnghlp.UsersGroups_Field_Access_MailingLists}
							<div id="groupRecord_record_listadmin_resources" class="groupRecord_Access_Resource_List">
								{if count($availableMailingLists) == 0}
									{$lang.UsersGroups_access_mailinglists_none}
								{else}
									<div {if $record.listadmin}style="display:none;"{/if}>
										<select name="record[access][lists][]" multiple="multiple" class="ISSelectReplacement ISSelectSearch">
											{foreach from=$availableMailingLists item=each}
					...
					<tr>
						...
						<td>
							<div>
								...
								<label for="recordGroup_Restriction_limit_list_flag">{$lang.UsersGroups_Restrictions_MailingLists_Unlimited}</label>
							...
							<div id="recordGroup_Restriction_limit_list_container" class="groupRecord_COMMON_NodeJoin" {if $record.limit_list == 0}style="display:none;"{/if}>
								{$lang.UsersGroups_Restrictions_MailingLists_Finite}
								<input type="text" class="Field50" value="{$record.limit_list}" name="record[limit_list]"/>
								{$lnghlp.UsersGroups_Restrictions_MailingLists_Finite}
							</div>
						</td>
					</tr>
					<tr>
