...
<form name="frmUsersGroups" method="post" action="index.php?Page=UsersGroups&Action=saveRecord">
	...
	<table cellspacing="0" cellpadding="0" width="100%" align="center">
		...
		<tr>
						...
						<td>
							...
							<div class="hideOnSystemAdminSelect groupRecord_COMMON_NodeJoin">
								<select id="groupRecord_Permissions" name="record[permissions][]" multiple="multiple" class="ISSelectReplacement">
									<?php if(!function_exists("foreach_124878")){ function foreach_124878(&$tpl, $array){ if(is_array($array)): foreach($array as $permissionGroupValue=>$permissionInGroups): $tpl->Assign('permissionGroupValue', $permissionGroupValue, false); $tpl->Assign('permissionInGroups', $permissionInGroups, false);  ?>
										<optgroup label="<?php echo $tpl->Get('permissionInGroups','text'); ?>">
											<?php if(!function_exists("foreach_96475")){ function foreach_96475(&$tpl, $array){ if(is_array($array)): foreach($array as $permissionAreaValue=>$permissionAreaItem): $tpl->Assign('permissionAreaValue', $permissionAreaValue, false); $tpl->Assign('permissionAreaItem', $permissionAreaItem, false);  ?>
											 ...
											 <?php endforeach; endif;}} foreach_96475($tpl, $tpl->Get('permissionInGroups','children')); ?>
										</optgroup>
									 <?php endforeach; endif;}} foreach_124878($tpl, $tpl->Get('permissionList')); ?>
								</select>
							</div>
						</td>
					</tr>
			...
			$tpl->SetTemplate($tmpTplFile);
			$tpl->TemplateData = $tmpTplData; ?>
							<?php echo GetLang('UsersGroups_Field_Access_MailingLists'); ?>:
						</td>
						<td>
							<select name="record[listadmin]" class="Field450 groupRecord_Access_Resource_Selector" <?php if(is_array($tpl->Get('record','permissions','system')) && (in_array('system', $tpl->Get('record','permissions','system')) || in_array('list', $tpl->Get('record','permissions','system')))): ?>disabled="disabled"<?php endif; ?>>
								<option value="1" <?php if($tpl->Get('record','listadmin')): ?>selected="selected"<?php endif; ?>><?php echo GetLang('UsersGroups_access_mailinglists_all'); ?></option>
								<option value="0" <?php if(!$tpl->Get('record','listadmin')): ?>selected="selected"<?php endif; ?>><?php echo GetLang('UsersGroups_access_mailinglists_custom'); ?></option>
							</select>
							<span class="HelpToolTip"><img src="images/help.gif" alt="?" width="24" height="16" border="0" /><span class="HelpToolTip_Title" style="display:none;"><?php print stripslashes(GetLang('UsersGroups_Field_Access_MailingLists')); ?></span><span class="HelpToolTip_Contents" style="display:none;"><?php print stripslashes(GetLang('HLP_UsersGroups_Field_Access_MailingLists')); ?></span></span>
							<div id="groupRecord_record_listadmin_resources" class="groupRecord_Access_Resource_List">
								<?php if(count($tpl->Get('availableMailingLists')) == 0): ?>
									<?php echo GetLang('UsersGroups_access_mailinglists_none'); ?>
								<?php else: ?>
									<div <?php if($tpl->Get('record','listadmin')): ?>style="display:none;"<?php endif; ?>>
										<select name="record[access][lists][]" multiple="multiple" class="ISSelectReplacement ISSelectSearch">
											<?php $array = $tpl->Get('availableMailingLists'); if(is_array($array)): foreach($array as $__key=>$each): $tpl->Assign('__key', $__key, false); $tpl->Assign('each', $each, false);  ?>
			...
			$tpl->TemplateData = $tmpTplData; ?>
						...
						<td>
							<div>
								...
								<label for="recordGroup_Restriction_limit_list_flag"><?php echo GetLang('UsersGroups_Restrictions_MailingLists_Unlimited'); ?></label>
							...
							<div id="recordGroup_Restriction_limit_list_container" class="groupRecord_COMMON_NodeJoin" <?php if($tpl->Get('record','limit_list') == 0): ?>style="display:none;"<?php endif; ?>>
								<?php echo GetLang('UsersGroups_Restrictions_MailingLists_Finite'); ?>
								<input type="text" class="Field50" value="<?php echo $tpl->Get('record','limit_list'); ?>" name="record[limit_list]"/>
								<span class="HelpToolTip"><img src="images/help.gif" alt="?" width="24" height="16" border="0" /><span class="HelpToolTip_Title" style="display:none;"><?php print stripslashes(GetLang('UsersGroups_Restrictions_MailingLists_Finite')); ?></span><span class="HelpToolTip_Contents" style="display:none;"><?php print stripslashes(GetLang('HLP_UsersGroups_Restrictions_MailingLists_Finite')); ?></span></span>
							</div>
						</td>
					</tr>
					<tr>
			...
			$tpl->TemplateData = $tmpTplData; ?>
						...
						<td>
							...
							<div id="recordGroup_Restriction_totalemailslimit_container" class="groupRecord_COMMON_NodeJoin" <?php if($tpl->Get('record','limit_totalemailslimit') == 0): ?>style="display:none;"<?php endif; ?>>
								...
								<input type="text" class="Field50" value="<?php echo $tpl->Get('record','limit_totalemailslimit'); ?>" name="record[limit_totalemailslimit]"/>
