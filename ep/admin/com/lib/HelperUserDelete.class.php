...
class HelperUserDelete
{
	...
	static private function _deleteUser($userid)
	{
		...
		if (($temp = self::_deleteFromSimpleTable('usergroups_access', 'groupid', $user->groupid)) === false) return $status; else $status['data']['usergroups_access'] = $temp;
		if (($temp = self::_deleteFromSimpleTable('usergroups_permissions', 'groupid', $user->groupid)) === false) return $status; else $status['data']['usergroups_permissions'] = $temp;
