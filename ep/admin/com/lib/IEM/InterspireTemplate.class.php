...
class IEM_InterspireTemplate extends InterspireTemplate
{
	...
	private function IEM_Menu()
	{
		...
		$unlimited_emails = $user->hasUnlimitedCredit();
		
		if (!$unlimited_emails) {
		    $creditLeft = API_USERS::creditAvailableTotal($user->userid);
		    
			$GLOBALS['TotalEmailCredits'] = sprintf(GetLang('User_Total_CreditsLeft'), SendStudio_Functions::FormatNumber($creditLeft));
		}

		$monthly_credits = $user->group->limit_emailspermonth;
		
		$GLOBALS['MonthlyEmailCredits'] = '';

		// The following should only be called when monthly_credit is set AND the page is not an upgrade page.
		if (($monthly_credits > 0) && (!isset($_GET['Page']) || strtolower($_GET['Page']) != 'upgradenx')) {
