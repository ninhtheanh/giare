...
class record_Users extends IEM_baseRecord
{
	...
	public function getUsedCredit($startDate = null, $endDate = null)
	{
	    ...
	    if ($startDate) {
	        $query .= ' AND queuetime >= ' . $startDate;
	    }
	    
	    if ($endDate) {
	        $query .= ' AND queuetime < ' . $endDate;
	    }
		...
		$result = $db->Query("
			SELECT
				COUNT(ls.subscriberid) AS count
			FROM
				[|PREFIX|]jobs             AS j,
		...
		$queue    = $db->Fetch($result);
		$credits += (int) $queue['count'];
