...
class Splittest_Stats_API extends API
{
	...
	{
		$query = "SELECT jobdetails from [|PREFIX|]jobs WHERE jobid = " . intval($job_id);
		//$rs = $this->Db->Query('SELECT sc.campaignid FROM [|PREFIX|]splittests s, [|PREFIX|]splittest_campaigns sc ' . 'WHERE s.splitid = ' . intval($splitid) . ' AND sc.splitid = s.splitid');
		$details = unserialize($this->Db->FetchOne($query));
		$campaign_ids = $details['newsletters'];
		return $campaign_ids;
	}
